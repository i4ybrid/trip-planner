 ...
 */
router.get('/trips/:tripId/calendar', auth_1.authMiddleware, async (req, res) => {
    try {
        const { tripId } = req.params;
        const events = await calendar_service_1.calendarService.getTripCalendarEvents(tripId);
 ...
 */
router.get('/trips/:tripId/calendar.ics', auth_1.authMiddleware, async (req, res) => {
    try {
        const { tripId } = req.params;
        const events = await calendar_service_1.calendarService.getTripCalendarEvents(tripId);
        const icsContent = calendar_service_1.calendarService.generateICal(events);
        res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="trip-calendar-${tripId}.ics"`);
        res.send(icsContent);
    }
    catch (error) {
        logger_1.logger.error('Error generating iCal:', error);
 ...
 */
router.get('/trips/:tripId/calendar/google', auth_1.authMiddleware, async (req, res) => {
    try {
        const { tripId } = req.params;
        const events = await calendar_service_1.calendarService.getTripCalendarEvents(tripId);
        if (events.length === 0) {
        ...
        const googleUrl = calendar_service_1.calendarService.generateGoogleCalendarUrl(firstEvent);
        res.json({
            data: {
                url: googleUrl,
        ...
        logger_1.logger.error('Error generating Google Calendar URL:', error);
 ...
 */
router.get('/trips/:tripId/calendar/outlook', auth_1.authMiddleware, async (req, res) => {
    try {
        const { tripId } = req.params;
        const events = await calendar_service_1.calendarService.getTripCalendarEvents(tripId);
        if (events.length === 0) {
        ...
        const firstEvent = events[0];
        const outlookUrl = calendar_service_1.calendarService.generateOutlookCalendarUrl(firstEvent);
        res.json({
            data: {
                url: outlookUrl,
                eventCount: events.length,
        ...
        logger_1.logger.error('Error generating Outlook Calendar URL:', error);
 ...
 */
router.get('/trips/:tripId/calendar/events/:eventId', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching calendar event:', error);
 ...
 */
router.get('/trips/:tripId/calendar/events/:eventId/ics', auth_1.authMiddleware, async (req, res) => {
    try {
        const { tripId, eventId } = req.params;
        const events = await calendar_service_1.calendarService.getTripCalendarEvents(tripId);
        const event = events.find(e => e.id === eventId);
        ...
        const icsContent = calendar_service_1.calendarService.generateICal([event]);
        res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
