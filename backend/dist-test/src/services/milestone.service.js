...
class MilestoneService {
     ...
     */
    async generateDefaultMilestonesFromToday(tripId, startDate, endDate) {
        ...
        for (const milestone of createdMilestones) {
            await timeline_service_1.timelineService.writeMilestoneToTimeline({
                id: milestone.id,
     ...
     */
    async generateIdeaMilestones(tripId, startDate) {
        const now = new Date();
     ...
     */
    async generateFinalPaymentMilestone(tripId, startDate) {
        const existing = await this.prisma.milestone.findFirst({
     ...
     */
    async generateSettlementMilestones(tripId, endDate) {
        const addDays = (base, days) => {
            const d = new Date(base);
            d.setDate(d.getDate() + days);
            return d;
     ...
     */
    async recalculateMilestones(tripId, newStartDate) {
     ...
     */
    async getMilestonesWithProgress(tripId) {
        const milestones = await this.prisma.milestone.findMany({
            where: { tripId },
            include: {
                completions: {
        ...
        return milestones.map((milestone) => {
            ...
            const completedCount = milestone.completions.filter((c) => c.status === 'COMPLETED').length;
            return {
                ...milestone,
                memberCompletions,
                completedCount,
     ...
     */
    async triggerOnDemandAction(_tripId, actionType, _sentById, _recipientIds, _message) {
     ...
     */
    async createMilestone(tripId, data) {
     ...
     */
    async createCustomMilestone(tripId, data) {
        const reminderAt = new Date(data.dueDate);
        reminderAt.setDate(reminderAt.getDate() - 3);
        const milestone = await this.prisma.milestone.create({
            data: {
    ...
    async deleteMilestone(milestoneId) {
        ...
        if (milestone) {
            await timeline_service_1.timelineService.deleteMilestoneTimelineEvents(milestoneId);
            await timeline_service_1.timelineService.upsertNeedsRefresh(milestone.tripId);
        }
    ...
    async updateMilestone(milestoneId, data) {
        ...
        if (data.isSkipped !== undefined)
            updateData.isSkipped = data.isSkipped;
        if (data.isHard !== undefined)
            updateData.isHard = data.isHard;
        ...
        if (existing &&
            ...
            await timeline_service_1.timelineService.updateMilestoneTimelineEvent(milestoneId, {
                effectiveDate: data.dueDate,
            });
            await timeline_service_1.timelineService.upsertNeedsRefresh(existing.tripId);
        }
        return updated;
    }
    /**
     ...
     */
    async updateMilestoneCompletion(milestoneId, userId, status, note) {
     ...
     */
    async getMilestoneProgress(tripId) {
        const members = await this.prisma.tripMember.findMany({
            where: { tripId, status: 'CONFIRMED' },
            include: {
                user: {
        ...
        const memberProgress = members.map((member) => {
            ...
            const completedMilestones = memberCompletions.filter((c) => c.status === 'COMPLETED').length;
        ...
        const overdueMilestones = activeMilestones.filter((m) => new Date(m.dueDate) < new Date() && !m.isSkipped);
        const completedMilestones = activeMilestones.filter((m) => m.completions.every((c) => c.status === 'COMPLETED'));
        return {
